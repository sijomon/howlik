  ...
  <div class="container">
    <div class="row"> @if (Session::has('flash_notification.message'))
      ...
      <div class="col-sm-9 page-content">
        <div class="inner-box">
			@if($type=='mybusinessbooking')
			<h2 class="title-2"><i class="icon-docs"></i>							@if(strtolower(Request::segment('1')) == 'ar')					{{t('Bookings on')}} {{ $business->title_ar }}				@else					{{t('Bookings on')}} {{ $business->title }}				@endif 
				<a href="{{lurl('account/mybusinesslistings')}}" style="float:right;" class="btn btn-default btn-xs">{{t('Go back')}}</a>
			</h2>
			@else
			<h2 class="title-2"><i class="icon-docs"></i> @lang('global.My Business Orders') </h2>	
			@endif
					  ...
					  <div class="eo-modal-box">
					   <button type="button" class="btn eo-close" data-dismiss="modal">&times;</button>
						...
						@if(strtolower(Request::segment('1')) == 'ar')									
							...
							@if($book->biz_loc != '')
								{{--*/ $unse = unserialize($book->biz_loc); /*--}}
								@if(isset($unse[0])){{--*/ $unse = $unse[0]; /*--}}@endif
								{{ $unse->ciname }}
							@else
								{{ $book->ciname }}
							@endif
...
<script type="text/javascript">
	...
	function upStatus(id, status){
		$("#st-holder"+id).html('<img src="/images/loading-new.gif" width="15">');
		$.ajax({
			url: "{{ lurl('account/book_action') }}",
			type: 'post',
			data: {'id':id, 'status':status},
			dataType:'json',
			success: function(data)
			{
				$("#stDv"+id).html(data['btn']);
				$("#st-holder"+id).html(data['res']);
				console.log("success");
				console.log(data);
				return false;					
			},
