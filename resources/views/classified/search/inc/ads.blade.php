...
<?php
	...
	?>
<div class="item-list"> @if ($ad->p_pack_id==2)
  ...
  <div class="col-sm-2 no-padding photobox">
    <div class="add-image"> <span class="photo-count"><i class="fa fa-camera"></i> {{ $countPictures }} </span> <a href="{{ $adUrl }}"> <img class="thumbnail no-margin" src="{{ $adImg }}" alt="img" data-no-retina/> </a> </div>
  </div>
  <div class="col-sm-7 add-desc-box">
    <div class="add-details">
      ...
      <?php
					...
					?>
      <span class="date_libu ner"><i class=" icon-clock"> </i> {{ $ad->created_at }} </span> @if (isset($cats) and !$cats->isEmpty())
...
href="{!! qsurl($country->get('icode').'/'.trans('routes.t-search'), array_merge(Request::except('c'), ['c'=>$liveCatParentId])) !!}"
      ...
      - <span class="item-location locat_11"><i class="fa fa-map-marker"></i>&nbsp; <a href="{!! qsurl($country->get('icode').'/'.trans('routes.t-search'), array_merge(Request::except(['l', 'location']), ['l'=>$ad->city_id])) !!}"
 class="info-link">{{ \App\Larapen\Models\City::find($ad->city_id)->name }}</a> {{ (isset($ad->distance)) ? '- ' . round($ad->distance, 2) . 'km' : '' }} </span> </span> </div>
  </div>
  <div class="col-sm-3 text-right price-box">
    <h4 class="item-price"> @if ($ad->price > 0)
      ...
      @if ($country->get('currency')->in_left == 0){{ $country->get('currency')->symbol }}@endif
      @endif </h4>
    @if ($ad->p_pack_id==3) <a class="btn btn-danger btn-sm make-favorite"><i class="fa fa-certificate"></i><span> {{ t('Top Ads') }} </span></a>&nbsp;
    @endif
    @if ($ad->p_pack_id==4) <a class="btn btn-danger btn-sm make-favorite"><i class="fa fa-certificate"></i><span> {{ t('Featured Ads') }} </span></a>&nbsp;
    @endif
    @if (Auth::check()) 
