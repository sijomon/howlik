	...
	<div class="container">
		<div class="row">
			...
			<div class=" col-sm-9 page-content">
				<div class="inner-box">
					<h2 class="title-2"> @lang('global.Events Activity Graph') </h2>
					<div id="head-div">
						@if(isset($event) && count($event) > 0)
						<div  class="pull-left" style="display: inline-block;">
							<select name="event" id="event" class="form-control">
								@foreach($event as $id => $title)
									<option value="{{ $id }}">{{ ucfirst($title) }}</option>
								@endforeach
							</select>
						</div>
...
@section('javascript')
		...
		$( document ).ready(function() {
			...
			$( ".period" ).on( "click", function() {
				...
				if($(this).val() == 1) {
					...
					$("#one-month").show();
					$("#one-year").hide();
					var ab = $("#event option:selected").val();
					getGraphMonth(ab);
	...
	<script language="javascript">
		...
		var id 		= $("#event option:selected").val();
		var chart	= AmCharts.makeChart("chartdiv",
		{
			"type": "serial",
			"dataLoader": {
				"url": "{{ lurl('/account/event/graph/day') }}/"+id
			},
		...
		function getGraphMonth(id) {
			...
			AmCharts.loadFile("{{ lurl('/account/event/graph/day') }}/"+id, {}, function(data) {
		...
		function getGraphYear(id) {
			...
			AmCharts.loadFile("{{ lurl('/account/event/graph/month') }}/"+id, {}, function(data) {
				chart.dataProvider = AmCharts.parseJSON(data);
				chart.validateData();
			});
			return true;
