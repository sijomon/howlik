...
<div class="row">
	<div class="col-md-8 col-md-offset-2">
		<!-- Default box -->
			...
			<a href="{{url('admin/subadmin1')}}"><i class="fa fa-angle-double-left"></i> {{ trans('backpack::crud.back_to_all') }} <span class="text-lowercase"></span></a><br><br>
		  ...
		  <div class="box">
			...
			@endif
			 ...
			 {!! Form::open( array('route' => 'subadmin1postaddajax', 'method' => 'POST', 'id' => 'ajax_user')) !!}
			   <div class="form-group">
				...
				@foreach($cat as $c)
						<option value="<?php echo $c->code;?>"><?php echo $c->asciiname;?></option>
				@endforeach
				</select>
			  </div>	
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" name="localname" placeholder="enter the location name in(In Arabic)."  />
			  </div>	      <!-- load the view from the application if it exists, otherwise load the one in the package -->
						<!-- text input -->
			  <div class="form-group">
				<label>Name</label>
				<input type="text" class="form-control" name="asciiname" placeholder="Enter the location name in(In English)." />
			  </div>	
			 
			  
			<div class="checkbox">
				<label>
				  <input type="checkbox" name="active" value="1" label="Active" type="checkbox"> Active
				</label>
			</div>
