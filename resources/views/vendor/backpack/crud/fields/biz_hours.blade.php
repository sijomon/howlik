...
<div class="info">
	...
	<div class="biz-hours" id="biz-hours">
        <div class="biz-hours-box">
        	@if(isset($field['value']) && $field['value']!='')
				...
				@foreach($bizhrs as $key => $value)
					...
					<div><span><b>{{$bizDayA[$bizhrsA[0]]}} </b></span><span>{{date("h:i A", $timeSt)}} </span><span>- </span><span>{{date("h:i A", $timeEd)}} {{($bizhrsA[2]=='00.00')?'(midnight next day) ':''}}</span><a href="#" class="rem-bh">Remove</a><input name="biz_hours[]" value="{{$value}}" type="hidden"></div>
				@endforeach
			@endif
		</div>
        <ul class="biz-hours-select">
            ...
            <li>{{--*/ $time = strtotime($date.' 12:00 AM'); /*--}}
