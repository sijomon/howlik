...
@if ($errors->any()) <div class="callout callout-danger"> <h4>Please fix</h4> <ul> @foreach($errors->all() as $error) <li>{{ $error }}</li> @endforeach </ul> </div> @endif
	...
	<div class="col-lg-12 file_holder" >
		...
		<div class="sub_cateholder">
			<a href="{{url('admin/subadmin1add')}}" class="btn btn-primary ladda-button" data-style="zoom-in">
			<span class="ladda-label"><i class="fa fa-plus"></i> Location</a>
		</div>
		...
		<table id="crudTable" class="table table-bordered table-striped display">
                    <thead>
                      <tr>
                          ...
                          <th>slno</th> <!-- expand/minimize button column -->
						  <th>Code </th> 
					...
					@if(!empty($cat))
					 ...
					 ?>
                      ...
                      <tr data-entry-id="{{ $q->id }}">
                        ...
                        <td>
                            <a href="{{url('admin/subadmin1Edit/'.$q->id)}}" class="btn btn-xs btn-default"><i class="fa fa-edit"></i> </a>
						 <a onclick="return confirm('Remove this location ?');" style="color:red" href="{{url('admin/subadmin1_delete/'.$q->id)}}" class="btn btn-xs btn-default " title="Delete Location">
