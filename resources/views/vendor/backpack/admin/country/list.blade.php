...
<section class="content-header">
  <h1> <span class="text-capitalize">Countries</span> <small>{{ trans('backpack::crud.all') }} <span class="text-lowercase"></span> {{ trans('backpack::crud.in_the_database') }}.</small> </h1>
  <ol class="breadcrumb">
...
<div class="box-header with-border">
  <div class="col-lg-12 file_holder" >
    <div class="sub_cateholder"> <a href="{{url('admin/subcategoryadd')}}" class="btn btn-primary ladda-button" data-style="zoom-in"> <span class="ladda-label"><i class="fa fa-plus"></i> Subcategory</span></a> </div>
  </div>
  ...
  <div class="box-body">
    <table id="crudTable" class="table table-bordered table-striped display">
      ...
      @if(!empty($conA))
      @foreach($cat as $key => $q)
      <tr data-entry-id="{{ $q->id }}">
        <!-- expand/minimize button column-->
        <td class="details-control text-center cursor-pointer"><i class="fa fa-plus-square-o"></i> </td>
        <td>{{ $key+1 }}</td>
        <td>{{$q->name}} </td>
        <td>{{$q->slug}}</td>
        <td><a href="{{url('admin/subcategoryEdit/'.$q->id)}}" class="btn btn-xs btn-default"><i class="fa fa-edit"></i> </a> <a onclick="return confirm('Remove this subcategory ?');" style="color:red" href="{{url('admin/subcategory_delete/'.$q->id)}}" class="btn btn-xs btn-default " title="Delete User"> <i class="fa fa-trash"></i> </a> </td>
      </tr>
      @endforeach
      @endif
      </tbody>
