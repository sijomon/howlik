...
class CrudController extends BaseController {
	...
	public function __construct()
	{
		...
		// BOF Code to set permission {Business Edit Permission for Admin 2} By Vin 9/5/18
		if(Auth::user()->id==2){
			if(!(isset($_POST) && sizeof($_POST)>0)){
				$allowedRoutes = array('admin/businessfix');
				if(!in_array($this->crud['route'], $allowedRoutes)){
					abort(403, 'Not allowed.');
				}
			}
		}
		// EOF Code to set permission {Business Edit Permission for Admin 2} By Vin 9/5/18
	}
	/**
	...
	public function index()
	{
		...
		if (isset($this->crud['orderby']) && is_array($this->crud['orderby'])){
			if(is_object($entryQ)){
				...
				$entryQ = $model::orderBy($this->crud['orderby']['field'], $this->crud['orderby']['type']);
			}
		}
		
		if (isset($this->crud['limit']) && $this->crud['limit']>0){
			if(is_object($entryQ)){
				$entryQ1 = $entryQ;
				$this->data['totalRows'] = $entryQ1->count();
				$this->data['entries'] = $entryQ->paginate($this->crud['limit']);
			}else{
				$this->data['entries'] = $model::paginate($this->crud['limit']);
			}
		}else{		
			if(is_object($entryQ)){
	...
	public function storeCrud(StoreRequest $request = null)
	{
		...
		*/
		...
		if(isset($this->crud['bizMod']) && $this->crud['bizMod'] == 1) {
			
			// BOF CREATE BUSINESS LOCATION TABLE QUERY 
			$businessTbl = array(
				...
				'location_id' => $request->input('subadmin1_code'),
				'city_id' => $request->input('city_id'),
				'zip_code' => $request->input('zip'),
				'lat' => $request->input('lat1'),
				'lon' => $request->input('lon1'),
			);
			$locationTbl = new BusinessLocation($businessTbl);
			$locationTbl->save();
			// EOF CREATE BUSINESS LOCATION TABLE QUERY 
		
			if(Session::has('offersNew')) {
			
				$offerIds = Session::get('offersNew');
				$offerArr = explode(',', $offerIds);
				for($i=0; $i < count($offerArr);$i++) {
					
					$offerBizIdUpdate	=	BusinessOffer::where('id', $offerArr[$i])->first();
					if(!empty($offerBizIdUpdate)) {
						
						$offerBizIdUpdate->update(['biz_id' => $item->id]);
					}
				}
				
				Session::forget('offersNew');
			}
			
			if(Session::has('BizPicNew')) {
			
				$picNewIds = Session::get('BizPicNew');
				$picNewArr = explode(',', $picNewIds);
				for($i=0; $i < count($picNewArr);$i++) {
					
					$picBizIdUpdate	=	BusinessImage::where('id', $picNewArr[$i])->first();
					if(!empty($picBizIdUpdate)) {
						
						$picBizIdUpdate->update(['biz_id' => $item->id]);
					}
				}
				
				Session::forget('BizPicNew');
			}
			
			if(Session::has('LocationsNew')) {
			
				$locNewIds = Session::get('LocationsNew');
				$locNewArr = explode(',', $locNewIds);
				for($i=0; $i < count($locNewArr);$i++) {
					
					$locBizIdUpdate	=	BusinessLocation::where('id', $locNewArr[$i])->first();
					if(!empty($locBizIdUpdate)) {
						
						$locBizIdUpdate->update(['biz_id' => $item->id]);
					}
				}
				
				Session::forget('LocationsNew');
			}
	...
	public function updateCrud(UpdateRequest $request = null)
	{
		...
		*/
		...
		if(isset($this->crud['bizMod']) && $this->crud['bizMod'] == 1) {
			...
			$locationTblE = BusinessLocation::where('biz_id', $request->input('id'))->where('base', 1)->first();
			
			if(!empty($locationTblE)) {
				
				// BOF UPDATE BUSINESS LOCATION TABLE QUERY 
				$businessTbl = array(
				...
				$locationTblE->update($businessTbl);
				// EOF UPDATE BUSINESS LOCATION TABLE QUERY 
			}
			else {
				...
				$locationTblN = new BusinessLocation($businessTbl);
				$locationTblN->save();
				// EOF CREATE BUSINESS LOCATION TABLE QUERY 
			}
