<?php
	...
	class MainController extends Controller {
		...
		public function GetBusinessAllss(HttpRequest $request) {
			...
			if( count($user) ) {
				...
				$isSave = $newOne->save();
					...
					if($language_code=='en'){
						...
						$note->type		=	'هدف';
						$note->title	=	'تمت إضافة حدث جديد بنجاح';
					}
		...
		public function PostCreateBusiness(HttpRequest $request) {
			...
			if( count($user) ) {
				...
				if($language_code=='en'){
					...
					$note->type		=	'الحجز';
					$note->title	=	'تمت إضافة نشاط تجاري جديد بنجاح';
				}
	...
	public function ReviewsList(HttpRequest $request) {
		...
		if( count($user) ) {
			...
			$reviews	=	DB::table('review')
								->select('id','user_id','user_name','biz_id','review','rating','expense','created_at')
								->where('user_id', $user->id)
							->get();
