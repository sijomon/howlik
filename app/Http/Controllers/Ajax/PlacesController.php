...
class PlacesController extends FrontController
{
    ...
    public function getLocations($code)
    {
		...
		$admin1_rows = SubAdmin1::where('code', 'LIKE', $code . '.%')->orderBy('asciiname')->get(['code', $colName.' as name']);
        if ($admin1_rows->count() == 0) {
           return response()->json(['error' => ['message' => "Error. Please select other Country.",], 404]);
		   // return response()->json(['error' => ['message' => $code,], 404]);
        }
    ...
    public function getSubLocations($code)
    {
        ...
        if ($count_admin2_cities == 0) {
            ...
            $admin1_cities = City::where('country_code', $country_code)->where('subadmin1_code', $admin_code)->orderBy('asciiname',
                'asc')->get(['id as code', $colName.' as name']);
            ...
            $admin2_rows = SubAdmin2::where('code', 'LIKE', $code . '.%')->orderBy($colName)->get(['code', $colName.' as name']);
