...
class BusinessfixController extends CrudController
{
    public $crud = array(
    
        "model" => "App\Larapen\Models\Businessfix",
        "entity_name" => "Business Listing",
        "entity_name_plural" => "Business Listings",
        "route" => "admin/businessfix",
        "reorder" => false,
						...
						foreach ($data as $key => $value) {
							if(trim($value['name'])!=''){
								...
								if (isset($catA->translation_of) && $catA->translation_of>0) {
									...
									$errorA[] = "Category Not exists (". trim($category). ")";
								}
								$country_code = '';
								$subadmin1_code = '';
								$city_id = 0;
								...
								if (isset($conA->code) && $conA->code!='') {
									...
									if (isset($subA->code) && $subA->code!='') {
										...
										if (isset($cityA->id) && $cityA->id!='') {
											...
											$errorA[] = "City Not exists (".trim($country).'->'.trim($region).'->'.trim($city).')';
										}
									}else{
										$errorA[] = "Region Not exists (".trim($country).'->'.trim($region).')';
									}
								}else{
									$errorA[] = "Country Not exists (".trim($country).')';
								}
