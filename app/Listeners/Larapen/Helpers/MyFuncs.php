...
class MyFuncs
	{
		...
		function do_curl($curl_opts)
		{
		  ...
		  if (! $curl_opts) {
			if ($debug) { $this->logit("do_curl:ERR:curl_opts is empty"); }
			return $retarr;
		  }
		  ...
		  if (! $ch) {
			if ($debug) { $this->logit("do_curl:ERR:curl_init failed"); }
			return $retarr;
		  }
		  ...
		  if (curl_errno($ch)) {
			...
			if ($debug) { $this->logit("do_curl:ERR:$errno:$errmsg"); }
			curl_close($ch);
			unset($ch);
			return $retarr;
		  }
		 ...
		 function callcontact_yahoo($consumer_key, $consumer_secret, $guid, $access_token, $access_token_secret, $usePost=false, $passOAuthInHeader=true)
		{
		  ...
		  $foo = new oauth_helper();
		  // compute hmac-sha1 signature and add it to the params list
		  $params['oauth_signature_method'] = 'HMAC-SHA1';
		  $params['oauth_signature'] =
			 $foo->oauth_compute_hmac_sig($usePost? 'POST' : 'GET', $url, $params,
									 $consumer_secret, $access_token_secret);
		
		  // Pass OAuth credentials in a separate header or in the query string
		  if ($passOAuthInHeader) {
			$query_parameter_string =  $foo->oauth_http_build_query($params, true);
			$header = $foo->build_oauth_header($params, "yahooapis.com");
			$headers[] = $header;
		  } else {
			$query_parameter_string =  $foo->oauth_http_build_query($params);
		  }
		...
		function get_access_token_yahoo($consumer_key, $consumer_secret, $request_token, $request_token_secret, $oauth_verifier, $usePost=false, $useHmacSha1Sig=true, $passOAuthInHeader=true)
		{
		  ...
		  if ($useHmacSha1Sig) {
			...
			$params['oauth_signature'] =
			  $foo->oauth_compute_plaintext_sig($consumer_secret, $request_token_secret);
		  }
		
		  // Pass OAuth credentials in a separate header or in the query string
		  if ($passOAuthInHeader) {
			$query_parameter_string =  $foo->oauth_http_build_query($params, false);
			$header = $foo->build_oauth_header($params, "yahooapis.com");
			$headers[] = $header;
		  } else {
			$query_parameter_string =  $foo->oauth_http_build_query($params);
		  ...
		  if (! empty($response)) {
			list($info, $header, $body) = $response;
			$body_parsed = $foo->oauth_parse_str($body);
