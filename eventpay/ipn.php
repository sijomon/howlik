<?php
		...
		function insert_data($request) {
			...
			if(isset($_GET['id']) && $_GET['id'] > 0) {
				...
				if (mysqli_query($con, $sql)) {
					...
					$select		=	"SELECT * FROM paypal_log WHERE id = '".$id."'";
					...
					if(count($select) > 0) {
						...
						if(count($set) > 0) {
							...
							$total	=	$set[ticket_quantity] *	$set[ticket_amount];
							mysqli_query($con,"INSERT INTO event_tickets (user_id,event_id,ticket_quantity,ticket_amount,total_amount,active) 
										VALUES ('". $set[usr_id] ."','". $set[eve_id] ."','". $set[ticket_quantity] ."','". $set[ticket_amount] ."','".$total."',1)");
	...
	$to 		=	'sreeharis@shrishtionline.com';    //  your email
	$body		=	"An instant payment notification was successfully recieved\n";
	$body 		.=	"from ".$payer_email."on ".date('Y-m-d G:i:s', time());
