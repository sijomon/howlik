...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<form method="post" align="center" action="http://www.howlik.com/api/dashboard" id="form">
			<h3>Business Graph</h3> <br>
			</div>
			<div><input type="text" name="apikey" id="apikey" placeholder="API KEY" value="707351522319778" required /></div>
			<div><input type="text" name="biz_id" id="biz_id" placeholder="BIZ ID" value="37" required /></div>
			<div><input type="text" name="month" id="month" placeholder="Month : 1 or 12" value="1" required /></div>
...
<script>
	function get() {
		...
		$.ajax({
			url: 'http://www.howlik.com/api/business/graph/view',
			type: 'POST',
			dataType:'json',
			data: { 'apikey' : $('#apikey').val(), 'biz_id' : $('#biz_id').val(), 'month' : $('#month').val() },
