...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<form method="post" align="center" action="http://www.howlik.com/api/update/event/get" id="form">
			<h3>Event Update GET</h3> <br>
			</div>
			<input type="text" name="apikey" id="apikey" placeholder="API KEY" value="208221515212483" autocomplete="on" />
			<div>
				<br>
			</div>
			...
			<input type="text" name="event_id" id="event_id" value="65" placeholder="EVENT ID" />
			<div>
				<br>
			</div>
		...
		<form method="post" align="center" action="http://www.howlik.com/api/update/event/post" enctype="multipart/form-data" id="form">
			<h3>Event Update POST</h3> <br>
			...
			Create Tickets :
			<label>
				<input type="radio" name="ticket_type" id="ticket_type_0" class="ticket_type" placeholder="TICKET TYPE" value="0" checked /> None
				<input type="radio" name="ticket_type" id="ticket_type_1" class="ticket_type" placeholder="TICKET TYPE" value="1" /> Free
				<input type="radio" name="ticket_type" id="ticket_type_2" class="ticket_type" placeholder="TICKET TYPE" value="2" /> Paid
			</label>
			<div>
				<br>
			</div>
			...
			Privacy :
			<label>
				<input type="radio" name="event_privacy" id="event_privacy_0" class="event_privacy" value="0" placeholder="EVENT PRIVACY" checked /> Public
				<input type="radio" name="event_privacy" id="event_privacy_1" class="event_privacy" value="1" placeholder="EVENT PRIVACY" /> Private
			</label>
			<div>
				<br>
			</div>
			...
			<input type="hidden" name="event_id" id="_event_id" value="" />
...
<script>
	...
	$('input#event_image').change(function(event) {
		var tmppath = URL.createObjectURL(event.target.files[0]);
		$("img#img_uploaded").fadeIn("fast").attr('src',tmppath);
	});
	...
	function get() {
		...
		$.ajax({
			url: 'http://www.howlik.com/api/update/event/load',
			type: 'GET',
			headers: {
				'X-CSRF-TOKEN': $('#token').val()
			},
			dataType:'json',
			data: { 'apikey' : $('#apikey').val(), 'language_code' : $('#language_code').val(), 'event_id' : $('#event_id').val() },
			success: function(data) {
				if(data) {
					...
					if( data.events ) {
						...
						$('input#_event_id').val(data.events.id);
						$('input#event_title').val(data.events.event_name);
						$('select#event_type').val(data.events.event_type_id);
						$('input#start_date').val(data.events.event_date +' '+ data.events.event_starttime);
						$('input#end_date').val(data.events.eventEnd_date +' '+ data.events.event_endtime);
						$('select#country_code').val(data.events.country_code);
						$('select#city_code').val(data.events.event_place);
						$('textarea#event_details').html(data.events.about_event);
						$('textarea#organization_details').html(data.events.org_description);
						
						if( data.events.social_share == 1 ) {
							$('input#social_share').attr('checked', 'checked');
							$('input#social_share').val(data.events.social_share);
						}
						
						$('input#ticket_type_'+data.events.ticket_type).attr('checked', 'checked');
						if( data.events.ticket_type == 1 ) {
							$('input#ticket_count').removeAttr('readonly');
							$('input#ticket_count').val(data.events.ticket_count);
						}
						else if( data.events.ticket_type == 2 ) {
							$('input#ticket_count').removeAttr('readonly');
							$('input#ticket_price').removeAttr('readonly');
							$('input#ticket_count').val(data.events.ticket_count);
							$('input#ticket_price').val(data.events.ticket_price);
						}
						
						$('input#event_privacy_'+data.events.privacy).attr('checked', 'checked');
						if( data.events.privacy == 1 ) {
							$('input#visible_to').removeAttr('readonly');
							$('input#visible_to').val(data.events.visible_to);
						}
						
						$('img#img_uploaded').attr('src', 'http://www.howlik.com/'+data.events.event_image1);
					}
				}
			}
		});
