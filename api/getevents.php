...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<form method="post" align="center" action="http://www.howlik.com/api/events/upcome" id="form">
			...
			<input type="text" name="apikey" id="apikey" placeholder="API KEY" value="213401520830744" required />
			<div>
				<br>
			</div>
			...
			<input type="number" name="event_id" id="event_id" min="1" placeholder="EVENT ID" value="28" />
			<div>
				<br>
			</div>
...
<script>
	function get() {
		...
		$.ajax({
			url: 'http://www.howlik.com/api/events/single',
			type: 'GET',
			headers: {
				'X-CSRF-TOKEN': $('#token').val()
			},
			dataType: 'json',
			data: { 'apikey' : $('#apikey').val(), 'country_code' : $('#country_code').val(), 'city_code' : $('#city_code').val(), 'event_id' : $('#event_id').val() },
