...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<form method="post" align="center" action="http://www.howlik.com/api/dashboard" id="form">
			<h3>Event Graph</h3> <br>
			</div>
			<div><input type="text" name="apikey" id="apikey" placeholder="API KEY" value="707351522319778" required /></div>
			<div><input type="text" name="eve_id" id="eve_id" placeholder="EVENT ID" value="43" required /></div>
...
<script>
	function get() {
		...
		$.ajax({
			url: 'http://www.howlik.com/api/event/graph/view',
			type: 'POST',
			dataType:'json',
			data: { 'apikey' : $('#apikey').val(), 'eve_id' : $('#eve_id').val(), 'month' : $('#month').val() },
